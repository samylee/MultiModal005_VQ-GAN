        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    g_loss = -torch.mean(disc_fake) # disc_fake = disc_real (training), -1 -> 1, loss going down
                    λ = self.vqgan.calculate_lambda(perceptual_rec_loss, g_loss)

                    vq_loss = perceptual_rec_loss + q_loss + disc_factor * λ * g_loss

                    d_loss_real = torch.mean(F.relu(1. - disc_real)) # disc_real: -1 -> 1, loss going down
                    d_loss_fake = torch.mean(F.relu(1. + disc_fake)) # disc_fake: 1 -> -1, loss going down
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset-path', type=str, default='../datasets/flowers/train/rose', help='Path to data (default: /data)')
